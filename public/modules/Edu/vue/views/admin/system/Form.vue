    ...
    <div>
        ...
        <el-form :model="form" ref="form" label-width="100px" label-position="right" :inline="false" size="normal">
            ...
            <el-card shadow="never" :body-style="{ padding: '20px' }" class="mt-3">
                ...
                <x-lesson :lessons.sync="form.lessons" :width="100" #default="{ lesson }">
                    <el-button-group>
                        <el-button type="danger" size="mini" @click="delLesson(lesson)">删除</el-button>
                    </el-button-group>
                </x-lesson>
                <x-lesson-select @select="addLesson" class="mt-3" />
            </el-card>
...
export default {
    ...
    async created() {
        if (this.id) this.form = await axios.get(`system/${this.id}`)
    },
    methods: {
        //删除课程
        delLesson(lesson) {
