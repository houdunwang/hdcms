...
export const Login: FC<PropsWithChildren<AuthComponentProps>> = (props) => {
	const [loginType, setLoginType] = useState < 'name' | 'mobile' | 'email' > ('name')
	...
	const form = useAppForm({
		...
		onSubmit: async ({ value: body }) => {
			const { data } = await mutation.mutateAsync({
				body: { ...body, [loginType]: body[loginType] },
			})
			login(data.user)
		}
	})
			...
			<div className="grid xl:grid-cols-[400px_1fr] gap-6 w-full">
				<Card className={cn('flex flex-col gap-6')}>
					<CardHeader>
						...
						<div className="flex gap-2 mt-4">
							<Tabs
								...
								onValueChange={(v) => {
									setLoginType(v as 'name' | 'mobile' | 'email')
								}}
							>
					...
					<CardContent className="space-y-2 flex-1">
						...
						{loginType === 'email' && (
							<form.AppField name="email" type="email" children={(field) => <field.FieldInput label="邮箱" />} />
						)}
						{loginType === 'mobile' && (
							<form.AppField
								name="mobile"
