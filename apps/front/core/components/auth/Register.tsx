...
export function Register({ className, onSubmit, ...props }: LoginType) {
	const { api } = useApi()
	const { login } = useAuth()
		...
		<form autoComplete="off" onSubmit={e => {
			...
			<Card className={cn("flex flex-col gap-6", className)} {...props}>
				...
				<CardContent className="space-y-2">
					...
					<FieldDescription className="text-center text-sm gap-2 flex items-center justify-center">
						已有账号? <Link to="/login" className="text-primary hover:underline">登录</Link>
						<span className="text-muted-foreground">·</span>
						<Link to="/forget" className="hover:underline">找回密码</Link>
					</FieldDescription>
				</CardContent>
			</Card>
		</form>
