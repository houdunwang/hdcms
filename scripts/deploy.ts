...
async function main() {
	...
	try {
		...
		await cp(resolve(root, 'apps/admin/.env.production'), resolve(deployDir, 'admin/build/.env'))
	} catch (error) {
	...
	if (existsSync(frontSrc)) {
		...
		execSync('pnpm --filter @app/front deploy deploy/front --prod --legacy', {
			stdio: 'inherit',
			cwd: root
		})
		fse.copySync(resolve(deployDir, 'front/build'), resolve(deployDir, 'admin/build/public'), { overwrite: true })
	} else {
